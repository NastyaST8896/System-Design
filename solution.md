...
| `created_at`           | Дата создания записи.                  | Timestamp  | Да             | Текущее время при регистрации пользователя.                                          | Для аналитики.                                                                  |
...
| `model`                | Модель авто.                         | String     | Да             | Не может быть пустым. Максимум 100 символов.                                                                                             | Для привлечения клиентов. Позволяет группировать автомобили.                                                                                                                      |
...
| `user_id`               | Ссылка на пользователя.                                                                | Identifier  | Да             | FK                                                                                               | Связывает аренду с конкретным пользователем.                                                                                                                                                          |
| `car_id`                | Ссылка на автомобиль.                                                                  | Identifier  | Да             | FK                                                                                               | Связывает аренду с конкретным автомобилем.                                                                                                                                                            |
| `start_date`            | Дата и время начала аренды.                                                            | Timestamp   | Да             | Должен совпадать с датой создания аренды                                                         | Используется в расчете количества ожидаемых дней аренды.                                                                                                                                              |
| `end_date`              | Дата и время ожидаемого возврата автомобиля.                                           | Timestamp   | Да             | Не должен совпадать и быть раньше даты начала аренды                                             | Используется в расчете количества ожидаемых дней аренды. Участвует в расчете рейтинга пользователя.                                                                                                   |
| `actual_end_date`       | Фактическая дата и время возвращения автомобиля.                                       | Timestamp   | Нет            | Если указан, не должен совпадать и быть раньше даты начала аренды                                | Триггер для расчёта просрочки.                                                                                                                                                                        |
| `price_per_day_at_rent` | Цена автомобиля за сутки на момент аренды.                                             | Decimal     | Да             | До двух знаков после точки. Не может быть отрицательным.                                         | Для фиксации стоимости на момент аренды, чтобы она не менялась со временем.                                                                                                                           |
| `coefficient_at_rent`   | Множитель, который изменяет стоимость аренды.                                          | ENUM        | Да             | Только: 0.5, 1.0, 1.5.                                                                           | Для фиксации коэффициента на момент аренды, чтобы он не менялся со временем.                                                                                                                          |
| `total_price`           | Итоговая сумма на выбранное количество дней с учетом коэффициента.                     | Decimal     | Да             | До двух знаков после точки. Не может быть отрицательным и меньше цены аренды автомобиля за сутки | Для предоплаты.                                                                                                                                                                                       |
| `overdue_price`         | Итоговая сумма оплаты на просроченное количество дней с учетом коэффициента равного 2. | Decimal     | Нет            | До двух знаков после точки. Не может быть отрицательным. По умолчанию 0.                         | Для доплаты за просрочку возвращения автомобиля.                                                                                                                                                      |
| `final_price`           | Итоговая сумма оплаты включающая и `total_price` и `overdue_price`.                    | Decimal     | Нет            | До двух знаков после точки. Не может быть отрицательным.                                         | Для финансовой отчетности.                                                                                                                                                                            |
| `status`                | Статус аренды.                                                                         | ENUM        | Да             | Только: активна, завершена, досрочно прервана, просрочена, прервана по ДТП.                      | Для понимания сценария возврата. Для определения возможности аренды для конкретного пользователя или конкретного автомобиля. Для расчета рейтинга.                                                    |
| `created_at`            | Дата создания аренды.                                                                  | Timestamp   | Да             | Дата создания записи                                                                             | Для аналитики и отчётов.                                                                                                                                                                              |
...
| `accident_date` | Дата и время ДТП.             | Timestamp  | Да             |                                                |
| `severity`      | Cтепень повреждений.          | ENUM       | Да             | Легкая, средняя, тяжелая                       |
| `repair_price`  | Цена ремонта автомобиля       | Decimal    | Да             |                                                |
