...
### **Рейтинг пользователя (логика расчёта)**
   ...
   - **Данные для расчёта**: Общее число аренд ($S_{всех\_аренд}$) и сумма весов степеней тяжести ДТП ($S_{вес\_ДТП}$).
   ...
   - **Формула**:
     ...
     $$
     - Средний вес ($\frac{S_{вес\_ДТП}}{S_{всех\_аренд}}$) — показывает, сколько баллов повреждений в среднем приходится на одну аренду пользователя
     - $K = 33.33$, выбран так, чтобы при среднем весе ДТП = 3 (каждая аренда с severe) рейтинг падал до 0, а при среднем весе = 0 (без ДТП) оставался 100, обеспечивая пропорциональную шкалу от 0 до 100 для всех промежуточных значений..
 
### Таблица граничных сценариев расчёта рейтинга

| № | Сценарий                          | Данные                                          | Расчёт                                    | Рейтинг                                      | Бизнес-решение                                                                                                                                               | 
|:--|:----------------------------------|:------------------------------------------------|:------------------------------------------|:---------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1 | Новый пользователь без истории    | 0 аренд за 24 месяца                            | NULL                                      | Нет данных                                   | Не показывать рейтинг. Выводить сообщение: «Для расчёта рейтинга необходимо совершить хотя бы одну поездку». Это исключает манипуляции и даёт честный старт. |
...
| $Rating$                          | Если есть ДТП с severe в выборке то $Rating &le; 84$ |
...
**Где хранятся данные для рейтинга**:
  ...
  1. Статус аренды (active, completed) — в сущности Аренда. 
  2. Дата завершения аренды — в сущности Аренда. 
  3. Степень тяжести ДТП (minor, moderate, severe) — в сущности ДТП. 
  4. Вся бизнес-логика расчёта рейтинга — в коде приложения.

### **Оплата аренды (логика и атрибуты)**
   - **Порядок оплаты**: Пользователь определяет даты начала и окончания → оплата производится в день аренды.
   ...
   - **Просрочка возврата**:
     - Стоимость просроченных дней возрастает **в 2 раза**. 
   ...
   - **Формулы**:
     ...
     - $final\_price = total\_price + overdue\_price$

     - $total\_price = rental\_price\_per\_day\_at\_rent \times rentalDays \times coefficient\_at\_rent$

     - $rentalDays = end\_date - start\_date$

     - $coefficient\_at\_rent$ и $rental\_price\_per\_day\_at\_rent$ — замороженные значения на момент аренды.


   - **Фиксируемые атрибуты в сущности `Аренда` (замороженные/кэшированные)**:
   ...
   - **Базовая цена аренды за сутки** — хранится в сущности Автомобиль.

| Параметр                        | Значение                                                 |
|:--------------------------------|:---------------------------------------------------------|
| Коэффициент 0.5                 | $Rating$ $85–100$ → $цена \times 0.5$                    |
| Коэффициент 1.0                 | $Rating$ $30–84$ → $цена \times 1.0$                     |
| Коэффициент 1.5                 | $Rating < 30$ → $цена \times 1.5$                        |

### **Автомобиль (ограничения и статусы)**
...
### **ДТП (дополнительные уточнения)**
   - **Стоимость ремонта**: Определяется независимо от степени повреждения (т.е. фактическая сумма ремонта фиксируется отдельно). 


   - **Cтепень повреждения автомобиля**:
...
### **Общие архитектурные решения**
  ...
  - Кэшированные/материализованные поля (например, `total_price`, `final_price`) заполняются однократно и не пересчитываются (денормализация с заморозкой для истории). 
