...
| :--- | :--- | :--- | :--- | :--- |
| `user_id` | Уникальный идентификатор пользователя. Первичный ключ  | Identifier | Да | Генерируется автоматически |
...
| Атрибут | Описание | Тип | Обязательность | Допустимые значения |
| :--- | :--- | :--- | :--- | :--- |
| `profile_id` | Уникальный идентификатор профиля. Первичный ключ | String | Да | Генерируется автоматически |
| `user_id` |  ||||
| `first_name` |  ||||
| `patronymic` |  ||||
| `last_name` |  ||||
...
| Атрибут | Описание | Тип | Обязательность | Допустимые значения |
| :--- | :--- | :--- | :--- | :--- |
| `car_id` | Первичный ключ | String | Да |  |
| `vin` |  ||||
| `license_plate` |  ||||
| `model` |  ||||
| `year` |  ||||
...
| Атрибут | Описание | Тип | Обязательность | Допустимые значения |
| :--- | :--- | :--- | :--- | :--- |
| `user_id` | Первичный ключ | String | Да |  |
| `Профиль` |  ||||
| `Автомобиль` |  ||||
| `Аренда` |  ||||
| `ДТП` |  ||||
...
3. Автомобиль - Car: car_id, vin, license_plate, model, rental_price_per_day, parking_address.
4. Аренда - Rental: rental_id, user_id, car_id, start_date, end_date, actual_end_date, rental_price_per_day, total_price, status
5. ДТП - Accidents: accidents_id, car_id, rental_id, accident_date, severity
