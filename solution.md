...
### **Рейтинг пользователя (логика расчёта)**
   ...
   - **Период расчёта**: Скользящее окно — последние 24 календарных месяца (от текущей даты по дате окончания аренды).


   - **Учитываемые аренды**: Только завершённые.


   - **Рейтинг по умолчанию** (нет аренд): 100.


   - **Диапазон**: от 0 до 100 включительно.


   - **Данные для расчёта**: Общее число аренд (S_всехАренд) и сумма весов степеней тяжести ДТП (S_весДТП).
   ...
   - **Формула**:
     ...
     $$
     - K = 33.33 (коэффициент масштабирования: 100 / 3, где 3 — макс. вес). 
     - Если S_всехАренд = 0, рейтинг = 100. 
     - Если у пользователя 1 аренда с весом 3, то S_весДТП = 3, рейтинг = 0. 


   - **Коэффициент (coefficient) на основе рейтинга**:
     - < 30 → 1.5 
     - 30–69 → 1.0 
     - 70–100 → 0.5
